...
def show_text():
    ...
    try:
        with canvas(device) as draw:
            logger.info("Drawing on matrix")
            draw.text((0, -2), msg, fill="white")
            time.sleep(5)
            server_requ = True
            logger.info(f"Updated message to: {msg}")
            return {"status": "success", "updated_to": msg}, 200
...
def show_status():
    ...
    while True and not server_requ:
        ip = get_ip()
        temp = get_temp()
        msg = f"IP:{ip} T:{temp}"
