...
class PersonTracker:
    def __init__(
        self,
        confirm_time=0.7,     # seconds person must persist
        lost_time=1.0,        # seconds before considered gone
        speak_cooldown=10.0   # seconds
    ):
    ...
    def update(self, detected: bool):
        ...
        if detected:
            ...
            if not self.confirmed and now - self.first_seen >= self.confirm_time:
                self.confirmed = True
                events.append("CONFIRMED")

        else:
            self.first_seen = None
            if self.confirmed and self.last_seen and now - self.last_seen >= self.lost_time:
                self.confirmed = False
                self.last_seen = None
                events.append("LOST")
    ...
    def can_speak(self):
        return time.time() - self.last_spoken >= self.speak_cooldown
...
def is_person(dets, min_area=0.02, frame_area=FRAME_WIDTH*FRAME_HEIGHT):
    for x1,y1,x2,y2,_,cid in dets:
        if cid == 0:
            if (x2-x1)*(y2-y1) / frame_area > min_area:
                return True
    return False
...
def mjpeg():
    ...
    while True:
        ...
        detected = any(cid == 0 for *_, cid in dets)
        events = person_tracker.update(detected)

        if "CONFIRMED" in events and is_person(dets):
            logger.info("Person confirmed")
            led.show_red()

            if person_tracker.can_speak():
                voice_controller.sayText("Hello Albrecht")
                person_tracker.mark_spoken()

        elif person_tracker.confirmed:
