...
try:
    from hailo_platform import HEF, VDevice, InputVStream, OutputVStream
except:
    pass   # fallback will kick in
# -------- CONFIG ----------
HEF_PATH = "yolov/yolov8m.hef"   # <-- set your actual path
USE_HAILO = os.path.exists(HEF_PATH)
print(f"USE_HAILO: {USE_HAILO}")
...
class HailoDetector:
    def __init__(self, hef_path):
        ...
        self.logger = HailoLogger.getLogger("HailoDetector")
        ...
        if hef_path and os.path.exists(hef_path):
            try:
                ...
                self.logger.info("[Hailo] Using YOLOv8 HEF:", hef_path)
                self._use_hailo = True

            except Exception as e:
                self.logger.error("[Hailo] Init failed:", e)

        if not self._use_hailo:
            self.logger.info("[Detector] Using Haar fallback")
            xml =  xml = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
...
if __name__ == "__main__":
    led.show_blue()
    input("starting face hailo stream")
    try:
